...
### 📚 CFOP公式库
- **124个完整公式**：Cross(5) + F2L(41) + OLL(57) + PLL(21)
- 分类浏览和学习
- 公式图片演示（Assets.xcassets）
- 随时查看公式算法和描述
...
CubeTimer/
├── Models/
│   ├── Formula.swift              # 公式数据模型
│   ├── CompleteFormulaData.swift  # 124个CFOP公式数据
│   ├── Scramble.swift             # 打乱生成器
│   └── Solve.swift                # 成绩记录模型
│
├── Views/
│   ├── TimerView.swift            # 计时器界面
│   ├── FormulaListView.swift      # 公式列表浏览
...
│   ├── FormulaImages.imageset     # Cross公式图片
│   ├── F2LImages.imageset         # F2L公式图片
│   ├── OLLImages.imageset         # OLL公式图片
│   └── PLLImages.imageset         # PLL公式图片
│
